...
export async function GET(
  ...
  try {
    ...
    const totalBusinessValue = projectsWithPayments.reduce((sum: number, p: any) => sum + Number(p.quotedAmount || 0), 0);
    ...
    const outstandingAmount = projectsWithPayments.reduce((sum: number, p: any) => {
      ...
      return sum + Math.max(0, Number(p.quotedAmount || 0) - pReceived);
    }, 0);
